...
fun GamePlayScreen(
    ...
    Scaffold(topBar = {
        CenterAlignedTopAppBar(
            title = {
                val total = "${state.level.total}"
                val current = "${state.level.current}"
                Text(
                    fontWeight = Bold,
                    color = colorScheme.onSecondaryContainer,
                    text = stringResource(levels_step__gameplay).format(current, total),
                )
            },
        ...
        Box(
            ...
            Editor(
                ...
                modifier = Modifier
                    ...
                    .align { _, size, _ ->
                        IntOffset(
                            size.width.div(2).minus(editorWidthMiddle),
                            size.height.div(3).minus(editorHeightMiddle)
                        )
                    },
            )
